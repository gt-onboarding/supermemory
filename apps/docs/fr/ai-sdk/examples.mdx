---
title: "Exemples du SDK d’IA"
description: "Exemples complets montrant comment utiliser Supermemory avec Vercel AI SDK"
...
* IA : *Utilise l’outil addMemory* « J’ai mémorisé que vous êtes allergique aux cacahuètes et que vous adorez la cuisine italienne ! »
* Utilisateur : « Suggère un restaurant pour le dîner »
...
Créez un système d’assistance client qui se souvient de l’historique du client :
...
<CodeGroup>
  ...
  export async function POST(request: Request) {
    ...
    const result = await streamText({
      ...
      messages,
      system: `Vous êtes un assistant de documentation. Vous avez accès à toutes les
      conversations précédentes et pouvez vous référer aux échanges antérieurs. Aidez les utilisateurs à comprendre
      la documentation en vous appuyant sur le contexte précédent.`
    })
  ...
  // Point de terminaison dédié pour importer la documentation dans la memory
...
<div id="multi-user-learning-assistant">
  ## Assistant d’apprentissage multiutilisateur
</div>

Créez un assistant qui apprend auprès de plusieurs utilisateurs tout en conservant l’isolation des données :
...
Combinez le téléversement de fichiers avec les Memory Tools pour l’assistance à la recherche :
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    1. Retenir les modèles de programmation et préférences des conversations précédentes
    2. Rechercher dans les exemples de code et solutions antérieurs
...
Pour tous les exemples, assurez-vous d’avoir défini les variables d’environnement suivantes :
...
* Utilisez un contenu de memory descriptif pour améliorer la qualité des résultats de recherche
* Fournissez, dans vos prompts système, le contexte expliquant quand utiliser chaque outil
* Utilisez des en-têtes de projet pour distinguer les différents cas d’usage
* Mettez en place une gestion des erreurs en cas d’échec d’un outil

<div id="infinite-chat">
  ### Infinite Chat
</div>

* Utilisez des identifiants de conversation pour conserver des contextes de chat distincts
* Incluez des identifiants utilisateur pour des expériences personnalisées
* Testez avec différents providers pour trouver la meilleure solution pour votre cas d’usage
* Surveillez l’usage des tokens pour optimiser les coûts
...
* Mettez en place une authentification adaptée pour la production
* Prévoyez une limitation du débit pour les applications à fort volume
...
<CardGroup cols={2}>
  ...
  <Card title="Memory Router" icon="route" href="/fr/memory-router/overview">
    Proxy à intégrer tel quel pour les applications LLM existantes
  </Card>
</CardGroup>