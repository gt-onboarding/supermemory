---
title: "Integração com o AI SDK"
description: "Exemplos completos de como usar o Supermemory com o Vercel AI SDK para criar aplicações inteligentes"
---

Esta página apresenta exemplos abrangentes de uso do Supermemory com o Vercel AI SDK, cobrindo tanto a abordagem com Memory Tools quanto com Infinite Chat.
      ...
      <div className="flex flex-col h-screen max-w-2xl mx-auto p-4">
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          <input
            ...
            placeholder="Fale sobre você ou faça qualquer pergunta..."
...
* IA: *Usa a ferramenta addMemory* &quot;Memorizei que você é alérgico a amendoim e adora comida italiana!&quot;
...
Crie um sistema de suporte que lembre o histórico do cliente:
...
<CodeGroup>
  ...
  export async function POST(request: Request) {
    ...
    const result = await streamText({
      ...
      messages,
      system: `Você é um assistente de documentação. Você tem acesso a todas as
      conversas anteriores e pode se referir a discussões passadas. Ajude os usuários a entender
      a documentação aproveitando o contexto prévio.`
    })
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    system: `Você é um assistente de aprendizagem. Ajude os estudantes com seus trabalhos acadêmicos:
    1. Lembrando do progresso de aprendizagem e dificuldades deles
    2. Buscando informações relevantes de suas sessões anteriores
    3. Fornecendo explicações personalizadas baseadas no estilo de aprendizagem deles
...
Combine upload de arquivos com as Memory Tools para assistência em pesquisa:
...
<CodeGroup>
  ...
  export async function POST(request: Request) {
    ...
    const memory = await addMemory({
      apiKey: process.env.SUPERMEMORY_API_KEY!,
      content: file, // Supermemory faz o processamento do arquivo
      title: file.name,
      headers: {
...
Crie um assistente de programação que se lembre da sua base de código e das suas preferências:
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    3. Acompanhar arquitetura de projeto e decisões de design
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    messages,
    tools: {
      // Incluir ferramentas do Supermemory
...
Para todos os exemplos, certifique-se de que você tenha estas variáveis de ambiente:
...
* Use conteúdo de memory descritivo para obter melhores resultados de busca
* Inclua contexto nos prompts do sistema sobre quando usar cada ferramenta
...
* Teste com diferentes providers para encontrar a melhor opção para o seu caso de uso
...
* Use a busca para evitar memórias duplicadas
...
<CardGroup cols={2}>
  ...
  <Card title="Memory Router" icon="route" href="/pt-BR/memory-router/overview">
    Proxy de fácil integração para aplicativos LLM existentes
  </Card>
</CardGroup>