---
title: "AI SDK 集成"
description: "完整示例展示如何将 Supermemory 与 Vercel AI SDK 结合，构建智能应用"
---

本页提供了使用 Supermemory 搭配 Vercel AI SDK 的全面示例，涵盖 Memory Tools 与 Infinite Chat 两种方式。

<div id="personal-assistant-with-memory-tools">
  ## 集成 Memory Tools 的个人助理
</div>

构建一个能记住用户偏好和过往互动的 AI 助理：
      ...
      <div className="flex flex-col h-screen max-w-2xl mx-auto p-4">
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          <input
            ...
            placeholder="可以介绍一下你自己，或随便问我任何问题……"
...
* 用户：“我对花生过敏，而且我喜欢意大利菜”
* AI：*使用 addMemory 工具* “我已经记住你对花生过敏并且喜欢意大利菜！”
* 用户：“给晚餐推荐一家餐厅”
* AI：*使用 searchMemories 工具* “根据我对你的了解，我会推荐一家不含花生的意大利餐厅……”
...
<CodeGroup>
  ...
  export async function POST(request: Request) {
    ...
    const memory = await addMemory({
      apiKey: process.env.SUPERMEMORY_API_KEY!,
      content: file, // Supermemory 负责文件处理
      title: file.name,
      headers: {
...
将 Supermemory 工具与您自有的自定义工具结合使用：
...
* 使用更具描述性的 memory 内容，以获得更佳的搜索结果
* 在 system prompts 中加入关于何时使用各个工具的上下文说明
* 使用项目级标题区分不同用例
* 实施错误处理来应对工具失败
...
* 使用会话 ID 维护独立的聊天上下文
* 传递用户 ID 以提供个性化体验
* 结合不同的 provider 进行测试，寻找最适合你用例的方案
* 监控 token 的使用量以优化成本
